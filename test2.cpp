...
static void inline assert_pixels(GContext* ctx, GPixel pixel) {
	...
	for (int y = 0; y < bm.fHeight; y++) {
		for (int x = 0; x < bm.fWidth; x++){
			printf("%d", pixel);
			printf("%d", bm.fPixels[y * (bm.fWidth + bm.fRowBytes/4) + x]);
			printf("%c",'\n');
			...
			printf("%c",'\n');
			assert (bm.fPixels[y * (bm.fWidth + bm.fRowBytes/4) + x] == pixel);
}
...
#define PIXELS_SET_PER_TEST 1000000
...
int main(int argc, char** argv) {
    GPixel* storage = new GPixel[103 * 100];
    GBitmap bitmap;
    bitmap.fWidth = 100;
    bitmap.fHeight = 100;
	...
	memset(storage, 0, sizeof(GPixel) * 103 * 100);
    ctx->clear(color);
	assert_pixels(ctx, pixel);
    delete ctx;
